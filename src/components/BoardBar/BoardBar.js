  ...
  const onSubmitInvitation = (data) => {
    ...
    inviteUserToBoardAPI({ inviteeEmail, boardId })
      .then(invitation => {
        ...
        // mời một ngừoi dùng vào board xong sẽ bắn/gui sự kiện lên trên server
          ...
          <Col md={10} sm={12} className="col-no-padding">
            <div className="board-info">
              <div className="item board-logo-icon">
                ...
                <strong>DaoDuyDien MERN Stack</strong>
              </div>
              ...
              {/* Khi code tới phần hiển thị thông tin user sẽ sửa lại tên class member__avatars */}
              <div className="item member__avatars">
                {board?.users.map((u, index) => {
                  //  hiện ra 2 user: index là 0 - 1 - 2
                  if (index <= 2 ) {
                    return (
                ...
                {/* Số user còn lại sẽ dược hiện dứoi number */}
