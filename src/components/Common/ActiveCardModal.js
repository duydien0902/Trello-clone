...
function ActiveCardModal() {
  ...
  const beforeUpdateCardCover = (event) => {
    ...
    toast.promise(
      updateCard(reqData).finally(() => event.target.value = ''),
      { pending: 'Updating...' }
    )
  }
  ...
  const beforeUpdateCardComment = (e) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      ...
      const newComment = {
        ...
        userDisplayName: currentUser?.dislayName,
        content: e.target?.value
      }
      updateCard({ newComment }).then(() => e.target.value = '')
    }
  }
  ...
  const updateCard = async (updateData) => {
    ...
    //B1: cập nhật lại caí card đang active trong modal hiện tại
    dispatch(updateCurrentActiveCard(updatedCard))


    //B2: cập nhật lại cái bản ghi card trong active Board (nested data)
