...
import { CARD_MEMBERS_ACTION_REMOVE, CARD_MEMBERS_ACTION_PUSH } from '../utilities/constants'
...
const cardCollectionSchema = Joi.object({
  ...
  comments: Joi.array().items({
    ...
    content: Joi.string(),
    createdAt: Joi.date().timestamp() // Chỗ này sau sẽ dùng hàm $push khi thêm comment nên nó không ăn giá trị default Date.now giống hàm insertOne được
  }).default([]),
...
const updateMembers = async (cardId, incomingMember) => {
  try {
    ...
    if (incomingMember.action === CARD_MEMBERS_ACTION_REMOVE) {
      // $pull  trong mongodb để lấy 1 phần tử ra khỏi mảng rồi xoá nó đi
