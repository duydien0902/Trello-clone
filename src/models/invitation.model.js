...
const invitationCollectionSchema = Joi.object({
  inviterId: Joi.string().required(), // ngừoi mòi
  inviteeId: Joi.string().required(), // ngừoi được mời
  type: Joi.string().required().default(INVITATION_TYPES.BOARD_INVITATION), //INVITATION_TYPES at above
  boardInvitation: {
    boardId: Joi.string(),
...
const findByUser = async (userId) => {
  try {
    const results = await getDB().collection(invitationCollectionName).aggregate([
      { $match: {
        inviteeId: ObjectId(userId), //Timf theo inviteeId - người được mời
        _destroy: false
      } },
      { $lookup: {
      ...
      { $lookup: {
        from: BoardModel.boardCollectionName, // boards
